        ...
        <div class="el-picker-panel__body">
          <div class="el-date-range-picker__time-header" v-if="showTime">
            <span class="el-date-range-picker__editors-wrap">
              ...
              <span class="el-date-range-picker__time-picker-wrap el-date-range-picker__time-box" v-clickoutside="handleMinTimeClose">
                <el-input
                  ref="input"
                  :disabled="rangeState.selecting || !minVisibleTime"
                  @focus="handleMinTimeFocus('h')"
                  :value="minVisibleTime.substring(0,2)"
                  size="small"
                ...
                <el-input
                  ref="input"
                  :disabled="rangeState.selecting || !minVisibleTime"
                  @focus="handleMinTimeFocus('m')"
                  :value="minVisibleTime.substring(3,5)"
                  size="small"
                  @change.native="handleTimeChange($event, 'min')"/>
                <span> : </span>
                <el-input
                  ref="input"
                  :disabled="rangeState.selecting || !minVisibleTime"
                  @focus="handleMinTimeFocus('s')"
                  :value="minVisibleTime.substring(6,8)"
                  size="small"
                  @change.native="handleTimeChange($event, 'min')"/>
                <time-open ref="minTimePicker" 
                  @pick="handleMinTimePick" 
                  :visible.sync="minTimePickerVisible" >
                </time-open>
              </span>
            </span>
            ...
            <span class="el-date-range-picker__editors-wrap is-right">
              ...
              <span class="el-date-range-picker__time-picker-wrap el-date-range-picker__time-box" v-clickoutside="handleMaxTimeClose">
                <el-input
                  :disabled="rangeState.selecting || !maxVisibleTime"
                  @focus="handleMaxTimeFocus('h')"
                  :value="maxVisibleTime.substring(0,2)"
                  size="small"
                  @change.native="handleTimeChange($event, 'max')"/>
                <span> : </span>
                <el-input
                  :disabled="rangeState.selecting || !maxVisibleTime"
                  @focus="handleMaxTimeFocus('m')"
                  :value="maxVisibleTime.substring(3,5)"
                  size="small"
                  @change.native="handleTimeChange($event, 'max')"/>
                <span> : </span>
                <el-input
                  :disabled="rangeState.selecting || !maxVisibleTime"
                  @focus="handleMaxTimeFocus('s')"
                  :value="maxVisibleTime.substring(6,8)"
                  size="small"
                  @change.native="handleTimeChange($event, 'max')"/>
                <time-open ref="maxTimePicker" 
                  @pick="handleMaxTimePick" 
                  :visible.sync="maxTimePickerVisible" >
                </time-open>
                <!-- <el-input
                  size="small"
                  :disabled="rangeState.selecting"
...
export default {
    ...
    minTimePickerVisible(val) {
      if (val === true) {
        this.$nextTick(() => {
          ...
          this.$refs.minTimePicker.adjustSpinners('dateMin', this.maxDate)
        })
      }
    },
    ...
    maxTimePickerVisible(val) {
      if (val === true) {
        this.$nextTick(() => {
          ...
          this.$refs.maxTimePicker.adjustSpinners('dateMax', this.minDate)
        })
      }
    },
  ...
  methods: {
    ...
    // 最小时间时分秒选中
    handleMinTimeFocus(type) {
      this.$refs.minTimePicker.type = type
      // this.$refs.minTimePicker.maxVisibleTime = type
      this.minTimePickerVisible = true
    },
    // 最大时间时分秒选中
    handleMaxTimeFocus(type) {
      this.$refs.maxTimePicker.type = type
      this.maxTimePickerVisible = true
    }
  },

  components: { TimePicker, DateTable, ElInput, ElButton, TimeOpen }
}
</script>
